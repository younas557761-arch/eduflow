...
export default function StaffForm({ initial={}, onSubmit, busy=false }) {
  const [form,setForm]=useState({first_name:"",last_name:"",employee_no:"",role:"",email:"",phone:"",...initial});
  return <form onSubmit={e=>{e.preventDefault();onSubmit(form)}} className="grid gap-4 md:grid-cols-2">
    ...
    <button disabled={busy} className="btn-primary md:col-span-2">{busy?"Saving...":"Save staff"}</button>
  </form>;
}
