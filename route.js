...
export async function GET() {
 const supabase=await db(); const {data,error}=await supabase.from("staff").select("*").order("created_at",{ascending:false});
...
export async function POST(request) {
 ...
 const allowed="first_name,last_name,employee_no,role,email,phone".split(",");
