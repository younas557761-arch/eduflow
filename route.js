...
export async function GET() {
 const supabase=await db(); const {data,error}=await supabase.from("students").select("*").order("created_at",{ascending:false});
...
export async function POST(request) {
 ...
 const allowed="first_name,last_name,admission_no,date_of_birth,gender,class_id,guardian_name,guardian_phone".split(",");
